...
# ---------------------------------------------------------
# Department Fee Structure Model
# ---------------------------------------------------------
class DepartmentFeeStructure(Document):
    """Fee structure per department"""
    department: str                    # e.g. "Software Engineering"
    core_per_credit: int = 0           # Core course fee per credit hour
    elective_per_credit: int = 0       # Elective course fee per credit hour
    compulsory_per_credit: int = 0     # Compulsory course fee per credit hour
    lab_charges: int = 0               # Fixed lab charges per semester
    security_deposit: int = 0          # One-time / per-semester security deposit
    created_by: str = ""
    created_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))
    updated_at: datetime = Field(default_factory=lambda: datetime.now(timezone.utc))

    class Settings:
        name = "department_fee_structures"
        indexes = [
            IndexModel([("department", ASCENDING)], unique=True),
        ]
        keep_nulls = False


class DepartmentFeeStructureSchema(BaseModel):
    department: str
    core_per_credit: int = 0
    elective_per_credit: int = 0
