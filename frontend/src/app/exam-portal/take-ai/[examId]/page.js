...
export default function TakeAiExamPage() {
  ...
  const loadExam = async () => {
    ...
    try {
      ...
      if (errorMsg.includes('Safe Exam Browser') || errorMsg.includes('403')) {
        ...
        showError(errorMsg || 'Failed to load exam. Make sure you are enrolled in the correct class.');
  ...
  if (isSebLocked) {
    ...
    const configUrl = `${backendUrl}/ai-exams/${params.examId}/seb-config`;
    const launchUrl = `seb://localhost:3000/exam-portal/take-ai/${params.examId}`;
