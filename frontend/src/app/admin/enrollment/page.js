  ...
  const loadInitialData = async () => {
    ...
    try {
      ...
      for (const t of [...new Set(termsToCheck)]) {
        try {
          const s = await getRegistrationStatus(t);
          if (s?.status === 'OPEN') { statusData = s; break; }
          if (!statusData && s?.status) statusData = s;
        } catch {}
      }

      const [coursesData, studentsData] = await Promise.all([
        getCourses({}),
        listUsers('STUDENT', 0, 500)
      ]);

      setWindowStatus(statusData);

      const allStudents = studentsData?.users || studentsData || [];
      setStudents(allStudents);

      // Build unique departments from students
      const deps = [...new Set(allStudents.map(s => s.department).filter(Boolean))].sort();
      setDepartments(deps);

      // Filter courses by department
      let courseList = coursesData?.courses || coursesData || [];
      if (department !== 'ALL') {
        courseList = courseList.filter(c =>
          allStudents.some(s => s.department === department)
        );
      }
