...
import { User, Phone, MapPin, ShieldCheck, Lock, Camera, Check, AlertCircle, Loader2 } from 'lucide-react';
  ...
  return (
    <div className="p-3 md:p-4 max-w-5xl mx-auto min-h-screen">
      ...
      >
        {/* Header */}
        <div className="mb-3">
          <Badge variant="outline" className="badge-rose mb-1.5 text-xs">Account Settings</Badge>
          <h1 className="text-xl font-bold tracking-tight text-white font-heading">
            My Profile
          </h1>
          <p className="text-slate-400 mt-0.5 text-sm">View and manage your administrative information.</p>
        </div>
        ...
        {/* Hero Profile Card */}
        <Card className="glass-card border-white/5 overflow-hidden border-t-4 border-t-rose-500">
          <CardContent className="p-3.5 pt-3 relative sm:flex sm:items-center sm:gap-3.5">
            <div className="relative group inline-block">
              <Avatar className="w-14 h-14 border-4 border-[#0d1017] shadow-xl bg-background">
                ...
                <AvatarFallback className="bg-gradient-to-br from-rose-600 to-pink-600 text-white font-heading text-lg">
                  {profile.first_name?.[0]}{profile.last_name?.[0]}
                </AvatarFallback>
              </Avatar>
              <button className="absolute bottom-0 right-0 p-1 bg-rose-600 hover:bg-rose-500 text-white rounded-full shadow-lg transition-transform hover:scale-110" title="Upload coming soon">
            ...
            <div className="mt-2 sm:mt-0 flex-1 pb-1">
              ...
              <p className="text-rose-400 font-medium tracking-wide text-xs">@{profile.username}</p>
          ...
          <div className="lg:col-span-1 space-y-3">
            <Card className="glass-card border-white/5 h-full">
              ...
              <CardContent className="p-3.5 space-y-3.5">
                <InfoRow label="Username" value={profile.username || 'N/A'} Icon={User} />
                <InfoRow label="Admin Level" value={profile.admin_level || 'ADMIN'} Icon={ShieldCheck} />
              ...
              <CardContent className="p-3.5">
                ...
                {editing && (
                  <div className="flex justify-end gap-2.5 mt-4 pt-3 border-t border-white/5">
                    ...
                    <Button onClick={handleSave} disabled={saving} className="bg-rose-600 hover:bg-rose-500 text-white shadow-[0_0_15px_rgba(244,63,94,0.4)] h-8 text-sm">
      ...
      {editing ? (
        multiline ? (
          <Textarea
            ...
            className="bg-black/30 border-white/10 text-white focus-visible:ring-rose-500 min-h-[60px] resize-none text-sm"
          />
        ) : (
          <Input
            value={value}
            onChange={(e) => onChange(e.target.value)}
            placeholder={placeholder}
            className="bg-black/30 border-white/10 text-white focus-visible:ring-rose-500 h-9 text-sm"
          />
        )
      ) : (
