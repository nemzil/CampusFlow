  ...
  return (
    <Card className="glass-card border-white/5 bg-white/5 overflow-hidden group">
      <CardContent className="p-1.5 flex items-center gap-2">
        <div className={`p-1 rounded-md border transition-transform group-hover:scale-110 ${colorClass}`}>
          <Icon className="w-3 h-3" />
        </div>
        <div>
          <p className="text-[9px] font-semibold uppercase tracking-wider text-slate-400">{label}</p>
          <p className="text-sm font-bold text-white font-heading">{value}</p>
        </div>
      </CardContent>
    </Card>
  );
